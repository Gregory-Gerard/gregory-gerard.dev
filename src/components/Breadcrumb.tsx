...
export default function Breadcrumb(props: { pages: NonEmptyArray<{ name: string; url?: string | undefined }> }) {
      ...
      <nav className="flex px-5 py-3 rounded-2xl bg-zinc-800/50 shadow-outline" aria-label="Breadcrumb">
        <ol className="flex items-center space-x-1 md:space-x-3 min-w-0">
          <li className="flex items-center">
            <a href="/" className="flex items-center text-sm font-medium text-zinc-400 hover:text-zinc-50">
          ...
          {props.pages.map((item) => (
            <li key={item.name}>
              <div className="flex items-center">
                ...
                <a href={item.url} className="ml-1 text-sm font-medium md:ml-2 text-zinc-400 hover:text-zinc-50">
                  {item.name}
                </a>
              </div>
            </li>
          ...
          {lastPage && (
            <li aria-current="page" className="min-w-0">
              <div className="flex items-center">
                ...
                <span className="ml-1 text-sm font-medium md:ml-2 text-zinc-300 truncate">{lastPage.name}</span>
              </div>
            </li>
          )}
        </ol>
